...
class Slider extends BlockBase {
	...
	public function render( array $attributes, string $content, WP_Block $block ): string {
		...
		$arrows_html = $show_arrows
			? '<button class="gb-slider__prev" aria-label="' . esc_attr__( 'Previous', 'godevs-blocks' ) . '" type="button">' . $icon_prev . '</button>'
				. '<button class="gb-slider__next" aria-label="' . esc_attr__( 'Next', 'godevs-blocks' ) . '" type="button">' . $icon_next . '</button>'
			: '';
		...
		$pause_btn_html = $show_pause_btn
			? sprintf(
				...
				'</button>',
				esc_attr__( 'Pause slideshow', 'godevs-blocks' ),
				$icon_pause,
				$icon_play
			)
			: '';
